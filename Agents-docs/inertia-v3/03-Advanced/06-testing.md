...
Your application may have [SSR enabled](/v3/advanced/server-side-rendering) for development and production, but you might not need it during testing. You may set `INERTIA_SSR_ENABLED` to `false` in your `phpunit.xml` to prevent the Laravel adapter from dispatching SSR requests.

```xml theme={null}
<env name="INERTIA_SSR_ENABLED" value="false" />
```

Alternatively, you may use the `Inertia::disableSsr()` method in your test base class.
...
A boolean or closure may be passed to disable SSR conditionally. This is typically called from a service provider.
